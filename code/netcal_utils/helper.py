# from: https://github.com/EFS-OpenSource/calibration-framework/blob/main/examples/detection/Helper.py
...
def read_image_ids(filename: str) -> Tuple[List, List]:
    ...
    # assert fields "train" and "test_fixed_kde_cityscapes_fcos_0.5" within JSON file
    assert "train" in config, "Field \'train\' must be given in JSON file root for specifying training IDs."
    assert "test_fixed_kde_cityscapes_fcos_0.5" in config, "Field \'test_fixed_kde_cityscapes_fcos_0.5\' must be given in JSON file root for specifying testing IDs."

    train_ids = config["train"]
    test_ids = config["test_fixed_kde_cityscapes_fcos_0.5"]
